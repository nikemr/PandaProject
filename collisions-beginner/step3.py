...
"""
from panda3d.core import CollisionHandlerEvent, CollisionNode, CollisionSphere, CollisionTraverser
...
#**...but from now on things are about to change: these two functions are the handlers the collision handler will call as soon as two events will happens:
# the two objects touch each other, the two objects leave each other alone. Afterwards we'll see how to make this happen.
...
# this method is for the INTO event, as is when the FROM object (heart) collide with the INTO one (smiley). 
# You see that we set a pattern string with strange symbols inside 
# - I'm not going to dig these details cos you'll find a wide explanation inside the panda3D manual at the chapter "Collision Handlers" -
#  just know that that pattern will match a system event we'll define later, which the collision handler will call ASA the event occurs.
...
#** To manage the collision events need Directobject, because the chaining between the CollisionHandlerEvent handler and our two functions seen above, happens with the accept() function of 
# a DirectObject instance that define the event strings the task manager will call via the CollisionHandlerEvent stimulus.
DO=DirectObject()

#** And at last here it is the event catching setup: we tell the panda3D event manager to call our function handlers whenever 'somebody' fires the strings
#  'collider_heart-into-smileycnode' or 'collider_heart-out-smileycnode' - this is to say that whenever the CollisionHandlerEvent object detect a collision,
#  will send an event string to the engine formatted, as you can see, with first the name of the FROM collider (collider_heart precisely) the "-into-" or "-out-"
#  string chunk and at the end the name of the INTO collider (smileycnode precisely). Note that '-into-' and '-out-' are just for our convenience, nobody oblige 
# us to write them like that: we may call'em 'dummy' and 'foo' respectively but provided that they're specified also in the pattern declaration above in place of
#  '-into-' and '-out-'. Now go back above and compare what said here with the pattern strings specified above with the addInPattern and addOutPattern declarations
#  and with a little help of the panda3D manual you should understand all this knotted stuff.
